...
export default class data_filter extends data_clean {
    ...
    get_col(col, df = this.df) {
        return df.map(x => x[col])
    }

    // filter a column based on value array
    filter_col(col, val, df = this.df) {
        // col: col name
        // val: array
        var res = df.filter(item => {
            return val.includes(item[col])
        })
        return res
    }

    // filter a column based on rule
    filter_col_rule(col, rule, df = this.df) {
        var res = df.filter(item => {
            return eval('item[col]' + rule)
        })
        return res
    }
